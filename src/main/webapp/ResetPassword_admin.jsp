...
line-height: 25px;
 ...
 ResultSet rs;
	...
	String q="select * from admin where name like ('"+Query+"') or username like ('"+Query+"') or adminID like ('"+Query+"') or email like ('"+Query+"')";
	psd=con.prepareStatement(q); 	   
	   rs=psd.executeQuery();
	   if(rs.next()){
	%>
		
		  <form action="Validator_admin.jsp" method="post">
       <table>
		 <tr><td>Admin ID</td><td><input type="text" name="adminid" placeholder="Admin ID" required></td></tr>
